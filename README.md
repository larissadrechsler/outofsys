...
O HTML foi construído utilizando exclusivamente tags semânticas para garantir acessibilidade e SEO. O conteúdo principal é encapsulado em um `<main>` com um ID de âncora para o **Skip Link**. Utilizei também `<header>`, `<section>`, `<article>` e `<footer>`.
...
Interface viva e interativa para melhorar a experiência do usuário:
...
Utilizei Inteligência Artificial para estruturar a lógica inicial do formulário de registro e a seção de FAQ.
* **Refatoração:** Ajustei manualmente o código gerado para integrar as variáveis do meu Design System e corrigi seletores para garantir que o layout não ficasse centralizado de forma indesejada no desktop.
...
* **Font-Display:** Configurado (`swap`) para evitar o efeito de texto invisível durante o carregamento das fontes.
